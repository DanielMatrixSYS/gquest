...
--- CreateChild
---
function panel:CreateChild(title, id, func)
...
function panel:InsertChild(obj)
    self:CreateChild(obj.Name, obj.ID, function()
        local ui = vgui.Create("gQuest.QuestMainObjectivesEdit");
        ui:SetPos(self:GetWide() + 113, ((ScrH() / 2) - 508 / 2) - 50);
        ui:SetSize(0, 508);
        ui:SizeTo(349, 508, .25, 0);
        ui:SetObjective(obj.ID);
...
function panel:Paint(w, h)
    ...
    gQuest.DrawTextOutlined("Main/Quest Obj Editor/Edit", "gQuest.16", 7, 9, gQuest.White, gQuest.Black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER);
end
vgui.Register("gQuest.QuestMainObjectiveList", panel, "EditablePanel");