...
function panel:PostInit()
    ...
    for k, v in ipairs(LocalPlayer().gQuest_NPC_Table) do
        if (tonumber(v.ID) == self.NPC) then
            self.FoundNPC = v;
    ...
    if (not self.FoundNPC) then
        return;
    end

    local npc = self.FoundNPC;
    self.PanelData = {
        {
            name = "Header",
            example = npc.Header,
...

        {
            ...
            example = npc.IntSound or "vo/npc/male01/hi01.wav",
            editable = true,
        },

        {
            name = "Header Color",
            example = npc.HeaderColor,
            editable = true,
        },

        {
            name = "Sub Header Color",
            example = npc.SubHeaderColor,
            editable = true,
        },
...
function panel:Paint(w, h)
    ...
    local id = self.FoundNPC and self.FoundNPC.ID or 0;
    gQuest.DrawTextOutlined("Main/Quest NPC Editor/Edit/" .. id, "gQuest.16", 7, 9, gQuest.White, gQuest.Black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER);
end
vgui.Register("gQuest.QuestMainNPCSEditNPC", panel, "EditablePanel");